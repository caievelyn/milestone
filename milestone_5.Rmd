...
In Teppei Yamamoto and Daniel Smith's paper, they explore whether voter preferences for politicians' characteristics impact their decision on who to vote for, and the magnitude of this effect.
All analysis for this paper, while it is still in progress, can be found at the working Github repo.^[Working repository can be found here: https://github.com/caievelyn/milestone]

**Description of Replication Paper**:

**Replicated Graphic/Table**:
Below, please find a replicated graphic that displays the difference in average treatment effects of certain variables based on a politician's gender (*Fig. 1*). *Fig. 2* displays a correlation matrix, the likes of which can be found in the Appendix. *Fig. 3* is an image file that is a screenshot of the replication paper's Appendix, for ease of comparison to the replicated graph in *Fig. 2*.
...
```{r display, echo = FALSE, fig.width= 10, fig.height = 8.5}
